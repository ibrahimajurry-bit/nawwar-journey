...
export async function sendAccountApprovalEmail(to: string, teacherName: string): Promise<boolean> {
  const resend = getResendClient();
  if (!resend) return false;

  const fromEmail = "رحلة نوّار <info@nawwarjourney.qpon>";
  const loginUrl = "https://nawwarjourney.qpon";
...
<body style="margin:0;padding:0;font-family:'Segoe UI',Tahoma,Arial,sans-serif;background:#f0f4ff;">
  <div style="max-width:580px;margin:30px auto;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.08);">
    ...
    <!-- Header -->
    <div style="background:linear-gradient(135deg,#0f3d2e,#1a6b3c);padding:36px 24px;text-align:center;">
      <img src="https://d2xsxph8kpxj0f.cloudfront.net/310419663029980891/S9UfAnxEfs6upsP98hCzwU/nawwar-logo-AWuZdjrAyTDDSJocatLxwi.png"
           alt="Nawwar Journey"
           style="width:64px;height:64px;border-radius:14px;margin-bottom:14px;display:block;margin-left:auto;margin-right:auto;" />
      <h1 style="color:#ffffff;font-size:22px;margin:0 0 6px;font-weight:700;">
      ...
      <p style="color:rgba(255,255,255,0.7);font-size:13px;margin:0;">
        Account Approved — Nawwar Journey
      </p>
    </div>
    ...
    <div style="padding:32px 28px;">
      ...
      <p style="font-size:15px;color:#555;line-height:1.9;margin:0 0 20px;">
        يسعدنا إخبارك بأن حسابك في منصة <strong>رحلة نوّار</strong> قد تمت الموافقة عليه من قِبل المسؤول.
        يمكنك الآن تسجيل الدخول والبدء في استخدام جميع أدوات المنصة.
      </p>

      <!-- Success Box -->
      <div style="background:#f0fdf4;border-right:4px solid #16a34a;border-radius:10px;padding:14px 16px;margin-bottom:24px;">
        <p style="margin:0;font-size:14px;color:#166534;font-weight:600;">
          🎉 حسابك الآن مفعّل بالكامل — يمكنك البدء فوراً!
        </p>
      </div>

      <!-- Tools -->
      <p style="font-size:14px;color:#444;margin:0 0 12px;font-weight:600;">ما يمكنك فعله الآن:</p>
      <div style="display:flex;flex-direction:column;gap:10px;margin-bottom:24px;">
        <div style="background:#f8faff;border-radius:10px;padding:12px 16px;border-right:3px solid #1b5e8a;">
          <p style="margin:0;font-size:13px;color:#333;">🎮 <strong>منشئ الألعاب</strong> — أنشئ ألعاباً تعليمية تفاعلية لطلابك</p>
        </div>
        <div style="background:#f8faff;border-radius:10px;padding:12px 16px;border-right:3px solid #1a6b3c;">
          <p style="margin:0;font-size:13px;color:#333;">📚 <strong>مكتبة الألعاب</strong> — استعرض الألعاب المتاحة وشاركها</p>
        </div>
        <div style="background:#f8faff;border-radius:10px;padding:12px 16px;border-right:3px solid #f59e0b;">
          <p style="margin:0;font-size:13px;color:#333;">📱 <strong>مولّد QR</strong> — أنشئ رموز QR لمشاركة المحتوى</p>
        </div>
      </div>
      ...
      <div style="text-align:center;margin:24px 0;">
        <a href="${loginUrl}"
           style="display:inline-block;background:linear-gradient(135deg,#0f3d2e,#1a6b3c);color:#ffffff;text-decoration:none;padding:15px 44px;border-radius:12px;font-size:16px;font-weight:700;">
          ابدأ الاستخدام الآن ←
        </a>
      </div>
          ...
          <tr>
            ...
            <td style="vertical-align:middle;padding-right:12px;">
              ...
              <p style="margin:4px 0 0;font-size:12px;">
                <a href="https://nawwarjourney.qpon" style="color:#1a6b3c;text-decoration:none;">🌐 nawwarjourney.qpon</a>
                &nbsp;|&nbsp;
                <a href="mailto:info@nawwarjourney.qpon" style="color:#1a6b3c;text-decoration:none;">✉️ info@nawwarjourney.qpon</a>
              </p>
            </td>
          </tr>
        </table>
  ...
  try {
    const { data, error } = await resend.emails.send({
      from: fromEmail,
      to: [to],
      subject: "✅ تمت الموافقة على حسابك في رحلة نوّار",
      html: htmlContent,
    });

    if (error) {
      console.error("[Email] Resend approval email error:", error);
      return false;
    }
